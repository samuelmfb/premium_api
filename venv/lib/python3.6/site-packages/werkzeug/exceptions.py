...
class CustomerDisconnected(BadRequest):
    """Internal exception that is raised if Werkzeug detects a disconnected
    Customer.  Since the Customer is already gone at that point attempting to
    send the error message to the Customer might not work and might ultimately
    result in another exception in the server.  Mainly this is here so that
    it is silenced by default as far as Werkzeug is concerned.

    Since disconnections cannot be reliably detected and are unspecified
    by WSGI to a large extent this might or might not be raised if a Customer
    is gone.
...
class Unauthorized(HTTPException):
    ...
    this header, although real customers typically don't care.
...
class NotAcceptable(HTTPException):
    ...
    `Accept` headers of the Customer.
    """
...
class RequestedRangeNotSatisfiable(HTTPException):
    ...
    The Customer asked for an invalid part of the file.
...
class PreconditionRequired(HTTPException):
    ...
    customers attempting to update a resource through PUT or DELETE. By requiring
    each Customer to include a conditional header ("If-Match" or "If-Unmodified-
    Since") with the proper value retained from a recent GET request, the
    server ensures that each Customer has at least seen the previous revision of
    the resource.
    """
