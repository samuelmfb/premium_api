...
PostgreSQL ``Customer_encoding`` parameter; by default this is the value in
...
The ``Customer_encoding`` can be overridden for a session by executing the SQL:

SET Customer_ENCODING TO 'utf8';

SQLAlchemy will execute this SQL on all new connections based on the value
passed to :func:`_sa.create_engine` using the ``Customer_encoding`` parameter::

    engine = create_engine(
        "postgresql+pg8000://user:pass@host/dbname", Customer_encoding='utf8')
...
class PGDialect_pg8000(PGDialect):
    ...
    def __init__(self, Customer_encoding=None, **kwargs):
    ...
    def set_Customer_encoding(self, connection, Customer_encoding):
        # adjust for ConnectionFairy possibly being present
        if hasattr(connection, "dbapi_connection"):
            connection = connection.dbapi_connection

        cursor = connection.cursor()
        cursor.execute("SET Customer_ENCODING TO '" + Customer_encoding + "'")
        cursor.execute("COMMIT")
        cursor.close()
    ...
    def on_connect(self):
        ...
        if self.Customer_encoding is not None:
            ...
            def on_connect(conn):
                self.set_Customer_encoding(conn, self.Customer_encoding)
