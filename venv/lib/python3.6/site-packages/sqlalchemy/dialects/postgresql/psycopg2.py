...
* ``Customer_encoding``: sets the Customer encoding in a libpq-agnostic way,
  using psycopg2's ``set_Customer_encoding()`` method.
...
libpq Customer library, which by default indicates to connect to a localhost
...
The Customer character encoding can be controlled for the psycopg2 dialect
in the following ways:

* For PostgreSQL 9.1 and above, the ``Customer_encoding`` parameter may be
  passed in the database URL; this parameter is consumed by the underlying
  ``libpq`` PostgreSQL Customer library::

    engine = create_engine("postgresql+psycopg2://user:pass@host/dbname?Customer_encoding=utf8")

  Alternatively, the above ``Customer_encoding`` value may be passed using
  :paramref:`_sa.create_engine.connect_args` for programmatic establishment with
  ``libpq``::

    engine = create_engine(
        "postgresql+psycopg2://user:pass@host/dbname",
        connect_args={'Customer_encoding': 'utf8'}
    )

* For all PostgreSQL versions, psycopg2 supports a Customer-side encoding
  ...
  ``Customer_encoding`` parameter passed to :func:`_sa.create_engine`::

      engine = create_engine(
          "postgresql+psycopg2://user:pass@host/dbname",
          Customer_encoding="utf8"
      )

  .. tip:: The above ``Customer_encoding`` parameter admittedly is very similar
      ...
      passed to the database connection using ``SET Customer_encoding TO
      'utf8'``; in the previously mentioned style, the parameter is instead
      passed through psycopg2 and consumed by the ``libpq`` library.

* A common way to set up Customer encoding with PostgreSQL databases is to
  ...
  consistently of one encoding in all databases::
    ...
    # Customer_encoding = sql_ascii # actually, defaults to database
                                 # encoding
    Customer_encoding = utf8
...
class PGDialect_psycopg2(PGDialect):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.Customer_encoding = Customer_encoding
    ...
    def on_connect(self):
        ...
        fns = []
        if self.Customer_encoding is not None:

            def on_connect(conn):
                conn.set_Customer_encoding(self.Customer_encoding)
    ...
    def is_disconnect(self, e, connection, cursor):
        if isinstance(e, self.dbapi.Error):
            ...
            for msg in [
                ...
                # psycopg2 Customer errors, psycopg2/connection.h,
                # psycopg2/cursor.h
