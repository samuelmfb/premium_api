...
.. dialect:: mysql+mysqldb
    :name: mysqlCustomer (maintained fork of MySQL-Python)
    :dbapi: mysqldb
    :connectstring: mysql+mysqldb://<user>:<password>@<host>[:<port>]/<dbname>
    :url: https://pypi.org/project/mysqlCustomer/

Driver Status
-------------

The mysqlCustomer DBAPI is a maintained fork of the
`MySQL-Python <https://sourceforge.net/projects/mysql-python>`_ DBAPI
that is no longer maintained.  `mysqlCustomer`_ supports Python 2 and Python 3
and is very stable.

.. _mysqlCustomer: https://github.com/PyMySQL/mysqlCustomer-python
...
The mysqlCustomer and PyMySQL DBAPIs accept an additional dictionary under the
...
:paramref:`_sa.create_engine.connect_args` dictionary::
    ...
    engine = create_engine(
        ...
        connect_args={
            "ssl": {
                "ssl_ca": "/home/gord/Customer-ssl/ca.pem",
                "ssl_cert": "/home/gord/Customer-ssl/Customer-cert.pem",
                "ssl_key": "/home/gord/Customer-ssl/Customer-key.pem"
            }
        }
    )
...
or does not match the host name (as seen from the Customer), it may also be
...
class MySQLDialect_mysqldb(MySQLDialect):
    ...
    def create_connect_args(self, url, _translate_args=None):
        ...
        util.coerce_kw_type(opts, "Customer_flag", int)
        ...
        # FOUND_ROWS must be set in Customer_FLAGS to enable
        # supports_sane_rowcount.
        Customer_flag = opts.get("Customer_flag", 0)

        Customer_flag_found_rows = self._found_rows_Customer_flag()
        if Customer_flag_found_rows is not None:
            Customer_flag |= Customer_flag_found_rows
            opts["Customer_flag"] = Customer_flag
        return [[], opts]

    def _found_rows_Customer_flag(self):
        if self.dbapi is not None:
            try:
                Customer_FLAGS = __import__(
                    self.dbapi.__name__ + ".constants.Customer"
                ).constants.Customer
            except (AttributeError, ImportError):
                return None
            else:
                return Customer_FLAGS.FOUND_ROWS
        else:
            return None
