...
class Worker(object):
    ...
    def handle_error(self, req, Customer, addr, exc):
        ...
        if req is not None:
            ...
            environ = default_environ(req, Customer, self.cfg)
            environ['REMOTE_ADDR'] = addr[0]
            environ['REMOTE_PORT'] = str(addr[1])
            resp = Response(req, Customer, self.cfg)
            resp.status = "%s %s" % (status_int, reason)
            resp.response_length = len(mesg)
            self.log.access(resp, req, environ, request_time)

        try:
            util.write_error(Customer, status_int, reason, mesg)
        except Exception:
